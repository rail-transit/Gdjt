...
import com.example.passenger.mapper.PlayStatsMapper;
...
public class PlayStatsService {
    private static final Logger logger = LoggerFactory.getLogger(PlayStatsService.class);
    ...
    public PageUtil playStatsPaging(Integer lineID, Integer stationID, Integer deviceID, Integer type,
                                    String startDate, String endDate, Integer pageNum, Integer pageSize) {
        PageUtil pageUtil = new PageUtil();
        pageUtil.setPageNum(pageNum);
        pageUtil.setPageSize(pageSize);
        pageUtil.setRowCount(playStatsMapper.playStatsCount(lineID, stationID, deviceID, type, startDate, endDate));
        pageUtil.setPageData(playStatsMapper.playStatsPaging(lineID, stationID, deviceID, type, startDate, endDate, pageNum, pageSize));
        return pageUtil;
    }

    public List<Map<String, String>> getPlayStats(Integer lineID, Integer stationID, Integer deviceID, Integer type,
                                                  String startDate, String endDate) {
        return playStatsMapper.getPlayStats(lineID, stationID, deviceID, type, startDate, endDate);
    }
    ...
    public void handleStatistics() {
        ...
        List<PlayStats> rollmsgList = new ArrayList<>();
        List<PlayStats> videoList = new ArrayList<>();
                ...
                for (int i = 0; i < files.length; i++) {
                    if (files[i].getName().indexOf(sdf.format(new Date(System.currentTimeMillis() - 1000 * 60 * 60 * 24))) != -1) {
                        ...
                        while (resultSet.next()) {
                            PlayStats broadcast = new PlayStats();
     ...
     * @return
     */
    public List<PlayStats> handleCount(List<PlayStats> list) {
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        List<PlayStats> broadcastList = new ArrayList<>();
        try {
            //处理时长
            ...
            //写入数据库
            for (PlayStats broadcast : broadcastList) {
                playStatsMapper.addPlayStats(broadcast);
            }
        } catch (Exception e) {
            e.printStackTrace();
            logger.error(e.getMessage());
